...
/* -------------------  SESSION CHECK ------------------- */
if (!isset($_SESSION['logged_in']) || $_SESSION['role'] !== 'Instructor' || $_SESSION['department_id'] != 3) {
...
if (isset($_POST['create_task'])) {
    ...
    $tstmt = $conn->prepare("
        INSERT INTO tasks (title, description, duration, instructor_id, department_id)
        VALUES (?,?,?,?,?)
    ");
    $fixed_dept_id = 3;
    $tstmt->bind_param("sssii", $title, $description, $duration, $inst_id, $fixed_dept_id);
    $tstmt->execute();
    ...
    header("Location: ".$_SERVER['PHP_SELF']."?msg=".urlencode("Task created successfully" . (!empty($selected) ? " and sent!" : "!")));
    exit;
}
...
if (isset($_POST['delete_task'])) {
    ...
    $up2 = $conn->prepare("UPDATE tasks SET instructor_id = 0 WHERE task_id=? AND instructor_id=?");
...
$stmt_adv = $conn->prepare("SELECT section FROM section_advisers WHERE instructor_id = ? AND department_id = 3");
...
if (!empty($advisory_sections)) {
    ...
    $sql_adv_stud = "
        ...
        WHERE s.department_id = 3 AND COALESCE(s.section, 'A') IN ($sections_in)
        ORDER BY s.lastname
    ";
...
// Organization data fetching and notification generation removed
...
<head>
    ...
    <title>Adviser Dashboard - College of Hospitality and Tourism Management</title>
    <!-- Bootstrap 5 CSS -->
                ...
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" class="text-start">
                    ...
                    <div class="mb-2">
                        ...
                        <div class="password-container">
                            <input type="password" name="current_password" class="form-control form-control-sm" required id="inst_hosp_current_password">
                            <i class="fas fa-eye-slash password-toggle-icon" onclick="togglePasswordVisibility(this, 'inst_hosp_current_password')"></i>
                        </div>
                    </div>
                    <div class="mb-2">
                        <label class="form-label small">New Password</label>
                        <div class="password-container">
                            <input type="password" name="new_password" class="form-control form-control-sm" required id="inst_hosp_new_password">
                            <i class="fas fa-eye-slash password-toggle-icon" onclick="togglePasswordVisibility(this, 'inst_hosp_new_password')"></i>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label small">Confirm New Password</label>
                        <div class="password-container">
                            <input type="password" name="confirm_password" class="form-control form-control-sm" required id="inst_hosp_confirm_password">
                            <i class="fas fa-eye-slash password-toggle-icon" onclick="togglePasswordVisibility(this, 'inst_hosp_confirm_password')"></i>
                        </div>
                    </div>
