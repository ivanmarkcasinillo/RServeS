...
// Get today's time_in from time_records (latest entry) for the form
$today = date('Y-m-d');
$stmt = $conn->prepare("SELECT time_in, session FROM time_records WHERE student_id = ? AND work_date = ? ORDER BY created_at DESC LIMIT 1");
$stmt->bind_param("is", $student_id, $today);
$stmt->execute();
$time_record = $stmt->get_result()->fetch_assoc();
$time_in_today = $time_record ? $time_record['time_in'] : ''; // empty string if none
                ...
                <div class="row g-4 mb-5">
                    ...
                    <div class="col-md-6">
                        <div class="content-card h-100 mb-0">
                            ...
                            <form method="POST" id="attendanceForm" class="row g-2">
                                <div class="col-6 col-md-12">
                                    <label class="form-label fw-bold small">Date</label>
                                    <input type="date" name="work_date" class="form-control form-control-sm" value="<?= date('Y-m-d') ?>" readonly>
                                </div>
                                <div class="col-6 col-md-12">
                                    <label class="form-label fw-bold small">Session</label>
                                    <select name="session" id="sessionSelect" class="form-select form-select-sm" required>
                                ...
                                <div class="col-6 col-md-12">
                                    <label class="form-label fw-bold small">Time In</label>
                                    <input type="time" name="time_in" id="timeIn" class="form-control form-control-sm" readonly>
                                </div>
                                
                                <input type="hidden" name="calculated_hours" id="calculatedHoursHidden">
                                
                                <div class="col-6 col-md-12 d-flex align-items-end">
                                     <div class="mb-0 text-primary fw-bold w-100 text-center border rounded p-1 bg-light" style="height: 31px; display: flex; align-items: center; justify-content: center;">
                                        <small class="me-1">Hours:</small> <strong><span id="hoursDisplay">0.00</span></strong>
                                    </div>
                                </div>

                                <div class="col-12 mt-2">
                                    <button type="submit" name="submit_time" id="submitBtn" class="btn btn-primary btn-sm w-100" disabled>
