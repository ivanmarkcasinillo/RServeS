...
input:focus,
select:focus {
    outline: 2px solid rgba(26, 79, 122, 0.18);
    box-shadow: 0 0 0 4px rgba(58, 142, 189, 0.14);
    transform: translateY(-1px);
}
...
button:hover {
    ...
    box-shadow: 0 12px 22px rgba(18, 63, 123, 0.22);
}
...
.auth-bg-visual img {
    ...
    animation: authKenburns 26s ease-in-out infinite alternate;
}
@keyframes authKenburns {
    0%   { transform: translate3d(-1%, -1%, 0) scale(1.06); }
    100% { transform: translate3d(1.4%, 1.6%, 0) scale(1.1); }
}
...
<body>
<div class="auth-bg-visual" data-image="uploads/index_background.png"></div>
<div class="container">
...
// Single background image with gentle Ken Burns effect for auth page
(function(){
  const slot = document.querySelector('.auth-bg-visual');
  if (!slot) return;
  const src = slot.getAttribute('data-image') || 'uploads/index_background.png';
  const img = new Image();
  img.alt = '';
  img.decoding = 'async';
